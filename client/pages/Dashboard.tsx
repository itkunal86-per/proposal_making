      ...
      <div className="container pt-6 pb-10">
        <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
          <div>
            ...
            <p className="text-muted-foreground">Insights across proposals, clients, AI usage, and revenue.</p>
          </div>
          <div className="flex items-center gap-2">
            {presets.map((p) => (
              <Button key={p.label} size="sm" variant="outline" onClick={() => setPreset(p.days, p.label)}>
                {p.label}
              </Button>
            ))}
            <Select onValueChange={(v) => { if (v === "custom") return; }}>
        ...
        <Card className="mt-6 p-6">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm text-muted-foreground">Total: {data ? `$${fmt(data.totals.revenue)}` : "—"}</span>
          </div>
